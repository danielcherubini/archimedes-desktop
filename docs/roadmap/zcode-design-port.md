...
1. **`src/index.css`** — replace the single `@import "tailwindcss";` line with the port of `/home/daniel/Coding/AI/ZCode/packages/ui/src/styles.css`, assembled as follows (ZCode line numbers in parentheses):
   ...
   - `@import "tw-animate-css";` and `@import "shadcn/tailwind.css";` — **required by the ported primitives** (ZCode's own comment at lines 28–41 documents the failure mode without them: the `@custom-variant` definitions in `shadcn/tailwind.css` map Radix's `data-state`/`data-orientation` onto the `data-active:`/`data-horizontal:` custom variants used by `tabs.tsx`/`separator.tsx`/`select.tsx`/`badge.tsx`, and `tw-animate-css` provides the `animate-in`/`fade-in`/`zoom-in` utilities used by `dialog.tsx`/`alert-dialog.tsx`/`dropdown-menu.tsx`/`collapsible.tsx` — without them those components render "class present, effect absent"; the shadcn layer also defines the `--color-muted`/`--color-muted-foreground`/`--color-ring` tokens the primitives reference and the `animate-collapsible-*` keyframes).
...
- [ ] All 16 primitives exist under `src/components/ui/` and import only React, `radix-ui`, `class-variance-authority`, `lucide-react`, and the local `cn` — no ZCode-internal imports (`@zcode/*`, i18n, telemetry, stores).
...
- [ ] Write `src/components/ui/braille-loader.test.tsx` (vitest fake timers):
  - **Determinism:** `generateFrames("typing", …)` (import from `@/lib/braille-loader`) returns the same `frames` array content on repeated calls (the `frameCache`), and `frames[0]` for `typing` matches `new RegExp('^[\\u2800-\\u28ff]{' + getVariantGridSize("typing")[0] + '$')` — braille codepoints U+2800–U+28FF (including the blank U+2800), **never ASCII spaces** (the field buffer maps mask 0 → `\u2800`, not `" "`).
  ...
  - **Reduced motion:** mock `window.matchMedia` to report `(prefers-reduced-motion: reduce)` matching; render; advance timers; assert the text stays `frames[0]` (static).
...
2. **`src/components/SidePane.tsx`** — new component, no props (it reads the stores itself):
   ...
   - **Resizable + collapsible:** a `useState` width (default 320, persisted to `localStorage` key `"side-pane-width"` on change — write-through, no debounce) and a `collapsed` boolean (persisted to `"side-pane-collapsed"`). A 4px drag handle on the frame's left edge: a `w-1` (4px) `cursor-col-resize` div with `onMouseDown` starting a drag (track `mousemove`/`mouseup` on `window` in a `useEffect`; new width = start width − delta, clamped 240–480px; while dragging, show a 2px `bg-foreground-subtlest/50` line — implement as the handle's `hover:`/drag-state styling; the hit area stays transparent otherwise). When `collapsed`, the frame renders at 0 width (hidden) — the re-open control is the header toggle (Task 6) — also expose an imperative open via a tiny module-level event (`window.dispatchEvent(new CustomEvent("archimedes:side-pane-open"))`) so the header toggle (a different component tree) can re-open it; `SidePane` listens for it.
   - **Tab bar** (`h-9 px-2`): a `Tabs` primitive (Task 1) with two `TabsTrigger`s — "Todos" and "Subagents" — `text-ui-sm`, `rounded-md`, active `bg-selected`; the selected tab in `useState` (persisted to `localStorage` key `"side-pane-tab"`, default `"todos"`). Each trigger shows a **count badge** when its tab has content and is not the active tab: Todos badge = the main-column todo count for the active session; Subagents badge = the subagent entry count.
...
4. **`src/components/SubagentPanel.tsx`** — restyle to the Subagents-tab treatment, **preserving every existing behavior** (the panel is the ONLY render site for subagent-session `confirm`/`password`/`ask`/permission requests — `ChatStream` renders those only for the ACTIVE session, and a subagent's session id is never active; an unrendered request would hang until the bridge timeout. The existing panel doc comment says exactly this — keep it): one card per subagent session (`rounded-xl border-card-border bg-card`, `gap-2` stack, replacing today's `rounded-md border-neutral-800 bg-neutral-900/40` sections):
   ...
   - **Dropped** (superseded by the new frame model): the old rail collapse/auto-expand behavior (the `SidePane` frame owns collapse now; the count badge on the tab trigger covers discoverability).

**Steps:**
- [ ] Create `src/components/TodoBoardPanel.test.tsx` (NO existing test file exists — verified) with the new-presentation cases: assert the progress header renders `N/M` for a fixture column with 1 done / 3 total and the `progress` indicator's `width` style = `33.33…%` (the ported `progress.tsx` normalizes `value` to 0–100 and applies it ONLY to the indicator's `width` style — it does NOT forward `value` to the Radix Root, so there is no `aria-valuenow` to assert; pass the percent value, not a fraction); assert the three indicator renderings (check icon + `text-success` for done, `◉` + `text-warning` for in-progress, `○` + `text-foreground-subtlest` for pending); assert the empty-state text "No todos yet"; assert the subagent-column indented sub-rows render for a fixture `subagents` column.
- [ ] Update `src/components/SubagentPanel.test.tsx` FIRST: ADD assertions for the new presentation (card header shows the agent name and the status chip class per status — fixture a running + a failed entry; the "No subagent sessions" empty state), and REWRITE the single "renders nothing when there are no entries" assertion to assert the empty-state text. KEEP every other existing assertion unmodified (the confirm/password/ask rendering, the compact stream, the dismiss button, the `working` state chip, the "1 awaiting" badge, the metrics line).
- [ ] Write `src/components/SidePane.test.tsx`: renders the two tab triggers ("Todos", "Subagents"); clicking "Subagents" swaps the content (the `SubagentPanel` empty state shows); the selected tab persists to `localStorage` (`"side-pane-tab"`); with a fixture todo column, the inactive Todos trigger shows the count badge; dispatching the `archimedes:side-pane-open` event un-collapses the pane.
- [ ] Run `pnpm test` — the updated/new tests fail (the new presentation/`SidePane` don't exist).
- [ ] Implement 1–4.
- [ ] Run `pnpm test` — all pass.
- [ ] Run `pnpm build` — green.
...
**Acceptance criteria:**
- [ ] The app renders three panes; `ChatStream` no longer renders `TodoBoardPanel`/`SubagentPanel` (their imports are removed from it — that happens in Task 6, but the panels are already hosted by `SidePane` here, so remove them from `ChatStream` in THIS task to avoid double-rendering).
- [ ] Pane width persists across a re-mount (localStorage); collapse/re-open works via the event; tab selection persists.
- [ ] Both restyled panels pass their suites: `TodoBoardPanel.test.tsx` (new) fully green; `SubagentPanel.test.tsx` green with all pre-existing assertions intact (only the "renders nothing" presentation assertion rewritten). No store or Tauri file was modified.
...
1. **`src/components/SpacesList.tsx`** — rebuild as the sidebar (fixed 260px, `bg-sidebar`, `flex flex-col`):
   - **Action cluster** (`p-3`, `border-b border-border/50`): two full-width buttons in the design-reference shape (`h-8 rounded-lg`, icon `size-4` + label `text-ui-base` + right `kbd` hint `text-ui-xs text-foreground-subtlest`, `hover:bg-surface-hover`): **New Session** (`MessageCirclePlus`, hint `⌘N`) and **Open Space** (`FolderOpen`, hint `⌘O`).
     - New Session behavior: the existing `startNewConversation` logic from `ChatStream` (agentId = live ?? storedMostRecent ?? firstAgentId, `startSession`, `addSession`, `addSpace`) — **extract it into the new `src/hooks/useStartNewConversation.ts` hook** (below) and call it from BOTH the sidebar (here) and `ChatStream` (which replaces its inline `startNewConversation` + the now-orphaned `newConversationAgentId`/`storedMostRecent`/`firstAgentId` locals, the `agents` state, and the `listAgents` `useEffect` — `tsconfig` has `noUnusedLocals`, so the locals must go with the function). `ChatStream`'s header "New conversation" button keeps working through the hook in THIS task (its removal is Task 6) — this is what keeps `pnpm build` green at this task's commit. When no Space is selected (`activeSessionId === null`), the button opens the Open Space dialog instead. Register a `⌘N`/`Ctrl+N` keydown listener (a `useEffect` on `window`, matching the key combo, invoking the same handler) — and `⌘O`/`Ctrl+O` for Open Space.
...
2. **`src/hooks/useStartNewConversation.ts`** — new hook (the extracted logic, verbatim semantics):
   ...
   ```
   Body = today's `ChatStream` logic moved in: `agentId = view.liveSession?.agentId ?? storedMostRecent?.agentId ?? firstAgentId` (the `agents` list fetched here via `listAgents` in a `useEffect`, `firstAgentId = agents?.[0]?.id ?? ""`); `startNewConversation` = `startSession(agentId, view.path)` → `addSession(info)` + `addSpace(info.cwd)` on success, error capture on failure. `view` is `undefined`-safe (no-op). `ChatStream` calls it with the active session's `view`; the sidebar calls it with the selected Space's view (top button) or a group's view (`+` buttons).
3. **`src/components/NewSpaceDialog.tsx`** — restyle to the `dialog` primitive (Task 1): `rounded-2xl` shell, `bg-popover border-popover-border`, the folder-picker form with `input`/`button` primitives (`text-ui-base`), destructive/primary per the design. Logic unchanged. (Its dynamic title is `"New space"` when no cwd is set, else `` `New conversation in ${base}` `` — the test below asserts the literal.)

**Steps:**
- [ ] Write `src/hooks/useStartNewConversation.test.ts` FIRST (TDD for the extraction): seed `useSessions` with `useSessions.setState({ spaces: […], sessions: […], historySessions: […], activeSessionId: "s1" })` — **NOT** `getState().setSpaces(…)` (that triggers boot auto-selection + `openSession` → unmocked `loadHistory` IPC); mock Tauri with the pattern every existing component test uses: `vi.mock("../lib/tauri", async () => { const actual = await vi.importActual("../lib/tauri"); return { ...actual, startSession: vi.fn().mockResolvedValue({ sessionId: "new1", agentId: "pi", cwd: "/tmp/ws", capabilities: { loadSession: false } }), listAgents: vi.fn().mockResolvedValue([{ id: "pi", name: "pi" }]), respondPermission: vi.fn(), respondBridgeRequest: vi.fn() }; })`. Assert: with a fixture view (live session `agentId: "a-live"` + a stored session `agentId: "a-stored"`), `startNewConversation` calls `startSession` with `"a-live"` (the fallback chain's first hit) and then `addSession`/`addSpace`; with a view whose live session is absent, it uses the stored session's `agentId`; with no agents loaded, it no-ops (empty `agentId`).
...
1. **`src/components/ChatStream.tsx`** — restructure to the conversation frame:
   ...
   - Frame: `flex-1 min-w-0 bg-background-alt rounded-xl m-1 flex flex-col` (the 4px inset per the design reference); remove the `TodoBoardPanel`/`SubagentPanel` imports + renders (Task 4 hosts them).
   - **Header** (`h-12 border-b border-border/50 p-2`, `flex items-center gap-2`): session title — derive from the first `kind: "user"` message of the active session (truncate to ~80 chars, `text-ui-base font-medium`, gradient-fade mask like the sidebar rows) falling back to the Space name (the existing `spaceTitle` logic); the **Space chip** (`bg-surface rounded-lg px-2 py-0.5`, folder icon `size-3.5` + base name `text-ui-sm`) — replace the current plain `spaceTitle` paragraph; the **session selector** (existing Live/`#1`/`#2` options logic, restyled as a `select` primitive `text-ui-base`); a **"…" `dropdown-menu`** (trigger: `MoreHorizontal` icon button) with items — Pause (live only, the existing `pause` handler), Resume (stored + `canResume`, the existing `resume` handler), New Session in this Space (the `useStartNewConversation` hook from Task 5 — the menu item calls it with the active session's `view`, exactly like the sidebar's top button); a right-aligned **side-pane toggle** icon button (`PanelRight` icon) that dispatches the `archimedes:side-pane-open` event when the pane is collapsed, or a collapse event (`"archimedes:side-pane-collapse"`) when open — add the collapse listener to `SidePane` (a 2-line addition to Task 4's component).
...
- [ ] Add a `ChatStream` test file `src/components/ChatStream.test.tsx` if absent (create it): seed a session with one user message → the header title derives from it (truncated); seed `inTurn` true + no bridge state → the working indicator renders the `BrailleLoader` (assert an element with `role="status"`); seed a bridge `agentState` of `blocked` → "Waiting for your input…" renders; seed a turn with a `diff` message → the `FileSummaryCard` renders with the right totals.
- [ ] Run `pnpm test` — the new/updated tests fail.
- [ ] Implement 1–8 (including the `SidePane` collapse-listener addition + the `ChatStream` panel removal + the `!activeSessionId` empty-state restyle — the `useStartNewConversation` hook was created in Task 5; this task just wires the "…" menu item to it).
- [ ] Run `pnpm test` — all pass.
- [ ] Run `pnpm build` — green.
...
1. **Composer block** (replace the existing `<div className="border-t border-neutral-800 p-3">…` block in `ChatStream.tsx`):
   ...
   - `textarea`: `resize-none text-ui-base bg-transparent outline-none` (drop the old `bg-neutral-900 border` styling — the shell carries the border), auto-growing (a `useEffect` on the value setting `height = auto` then `scrollHeight`, min 2 rows / max 6 rows via `max-h` + `overflow-y-auto`), placeholder = **the existing THREE-state logic** (`isLive` / `canResume` / else — see the current composer) **plus ONE new state** (live + working): live + idle → "Send a prompt…"; live + working → "Agent is working…" (NEW — the old code had no working variant); stored + resumable → "Paused — Resume to reconnect"; history-only → "This session is closed"; `disabled` when not live or while working.
...
2. **Validation sweep** (AGENTS.md merge gate):
   ...
   - Grep gate: `rg '(^|[" ])(bg|text|border|ring)-(neutral|sky)-' src --glob '*.tsx'` returns **no hits** (every ad-hoc Tailwind color class in components is replaced by a token utility). The token DEFINITIONS in `src/index.css` are explicitly EXEMPT — the verbatim ZCode token blocks contain `var(--color-neutral-50)`/`var(--color-sky-400)`-style values (e.g. `--color-background: var(--color-neutral-50)`), and those are the design system, not ad-hoc usage; do NOT "fix" them.
